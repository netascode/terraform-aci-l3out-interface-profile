    ...
    for int in var.interfaces : [
      for path in coalesce(int.paths, []) : {
        ...
        value = {
          ...
          floating_l3domain = path.physical_domain != null ? "phys-${path.physical_domain}" : (path.vmware_vmm_domain != null ? "vmmp-VMware/dom-${path.vmware_vmm_domain}" : "")
          elag              = path.elag != null ? path.elag : null
        }
      }
    ] if int.floating_svi == true
  ])
...
resource "aci_rest_managed" "l3extRsDynPathAtt" {
  ...
  dn         = "${aci_rest_managed.l3extVirtualLIfP[each.value.node].dn}/rsdynPathAtt-[uni/${each.value.floating_l3domain}]"
  class_name = "l3extRsDynPathAtt"
  content = {
    floatingAddr = each.value.floating_ip
    tDn          = "uni/${each.value.floating_l3domain}"
  }
}
...
resource "aci_rest_managed" "l3extVirtualLIfPLagPolAtt" {
  ...
  dn         = "${aci_rest_managed.l3extVirtualLIfP[each.value.node].dn}/rsdynPathAtt-[uni/${each.value.floating_l3domain}]/vlifplagpolatt"
  class_name = "l3extVirtualLIfPLagPolAtt"
}

resource "aci_rest_managed" "l3extRsVSwitchEnhancedLagPol" {
  for_each   = { for item in local.floating_paths : item.key => item.value if item.value.elag != null }
  dn         = "${aci_rest_managed.l3extVirtualLIfP[each.value.node].dn}/rsdynPathAtt-[uni/${each.value.floating_l3domain}]/vlifplagpolatt/rsvSwitchEnhancedLagPol-[uni/${each.value.floating_l3domain}/vswitchpolcont/enlacplagp-${each.value.elag}]"
  class_name = "l3extRsVSwitchEnhancedLagPol"
  content = {
    tDn = "uni/${each.value.floating_l3domain}/vswitchpolcont/enlacplagp-${each.value.elag}"
